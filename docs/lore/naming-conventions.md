...
| **the Buffer** | The error-correction layer that catches part of a failed Hail's wager. Surfaced on the Hail card as the effective return %, and as a short Sera-voiced "the Buffer caught it" banner the moment after a loss. The gambleCushion buff family feeds it. |
...
| **Patched Frame** | A Cycle Pattern. Five bands ride free at cycle open and the sweep gets five free re-tunes; after that every purchase and every re-tune bills double. |
| **Bare Wire** | A Cycle Pattern. Base listening runs lean (×0.6), but windows hold twice as long, land 25 % stronger, and every hail carries eight points further. |
| **Echo Loom** | A Cycle Pattern built for the network. Every placed Seed Relay contributes double; base listening drops to half. Live on the carrier you place, not the rig itself. |
| **Anchor** | A reinforcement driven into a ripened Seed Relay. The first Anchor buries the relay deep enough that ComDef stops finding it (discovery → none); every Anchor also amplifies its carrier. Rarity sets the depth — common relays can't be anchored, the higher bands take more. Reads as a shrinking pyramid of hex tiers on the cell. Break the relay down to pull the Anchors and recover half the Echoes spent. Verb: *anchor* a relay; the locked state is *anchored*. |
| **True North** | The baseline Cycle Pattern. No skew at all — base listening, windows, and bands all run at standard. The named form of the default first-cycle experience; offered from cycle 2 on as the "no trade-off this run" choice. |
| **Open Market** | A Cycle Pattern. Bands come cheap (purchase cost ×0.6) and the sweep runs almost free (re-tune cost ×0.5), but the base carrier runs a little lean (×0.85). Front-loaded: fill the Console fast. |
| **Steady Sky** | A Cycle Pattern. The cycle opens under a long, calm carrier — ×2 effective rate held for two hours — but short purchased windows barely register (strength ×0.5). A patience pattern. |
| **Hot Band** | A Cycle Pattern. The base carrier runs hot (×1.5) and every hail carries ten points further, but every band and every re-tune bills double. Loud rig, expensive sky. |
...
- 2026-05-17 — Reworked **Patched Frame** (5 free purchases + 5 free re-tunes, then ×2 cost on both), buffed **Bare Wire** (base ×0.6 not ×0.5, windows now also ×1.25 strength, hail luck +8 % not +5 %), and added **Echo Loom** — a fifth Cycle Pattern that doubles network contribution at the cost of half base listening. Pattern picker now badges previously-completed Patterns, and there's a new Achievement ("Every shape of the rig") for closing a cycle on each Pattern at least once.
- 2026-05-31 — Added a baseline Cycle Pattern, **True North** (all modifiers neutral — the named default first-cycle shape, offered as the "no trade-off" choice from cycle 2 on), and three new Patterns: **Open Market** (cheap bands + cheap sweep, slightly lean base), **Steady Sky** (a two-hour ×2 standing carrier, weak short windows), and **Hot Band** (hot base + far-carrying hails, double cost on bands and re-tunes). All four use only existing Pattern modifier fields. Added four "clean run" Achievements — reaching 1e30 / 1e60 Echoes in a cycle with no Hail used, and with no Window used (**No hand on the dice**, **No window opened**, **The straight line**, **The long quiet climb**).
- 2026-05-31 — Added the **Log** tab to the Names panel — Kalen's chronological transcript of every world that answered the carrier and every line that was said when contact landed. ("The Log" was already the in-world name for the contact record; this surfaces its full transcript.) Replaced the end-of-cycle finale lore line in the contact-progress strip with a **signal indicator**: a living waveform when the current cycle is Kalen's best-ever push, a flat line when it is not.
